...
onMounted(() => {
  nextTick(() => {
    drawChart()
    if (detailCanvasRef.value) {
      detailResizeObs = new ResizeObserver(() => drawChart())
      detailResizeObs.observe(detailCanvasRef.value)
    }
  })
})

onUnmounted(() => {
  if (detailResizeObs) detailResizeObs.disconnect()
})
